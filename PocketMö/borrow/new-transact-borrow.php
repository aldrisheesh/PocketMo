...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Get form data
    $borrow_date = $_POST['initial_date']; // This will be retained for new records
    ...
    $stmt = $conn->prepare("SELECT ID, Balance FROM borrowed WHERE Name = ? AND User_Id = ? AND Balance > 0");
    ...
    if ($result->num_rows > 0) {
        ...
        $update_stmt = $conn->prepare("UPDATE borrowed SET Balance = ?, Due_Date = ? WHERE ID = ?");
        $update_stmt->bind_param("ssi", $new_balance, $due_date, $existing_id);

        // Execute the update statement
        if ($update_stmt->execute()) {
            $_SESSION['message'] = 'Borrowed record updated successfully!';
        } else {
        ...
        $balance = $amount; // Set balance to the amount borrowed

        // Prepare and bind the SQL INSERT statement
        $insert_stmt = $conn->prepare("INSERT INTO borrowed (User_Id, Borrow_Date, Amount, Name, Payment_Amount, Balance, Due_Date) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $insert_stmt->bind_param("issssss", $userId, $borrow_date, $amount, $lender_name, $payment_amount, $balance, $due_date);

        // Execute the insert statement
        if ($insert_stmt->execute()) {
            $_SESSION['message'] = 'Borrowed record added successfully!';
        } else {
    ...
    header("Location: ./borrow-page.php"); // Change this to your desired redirect page
...
<body>
    ...
    <div class="modal">
        <form id="transaction-form" action="" method="POST" onsubmit="return handleFormSubmit();">
            ...
            <div class="input-group">
                ...
                <input type="text" id="lender-name" name="lender_name" placeholder="Enter Lender’s name" required>
            </div>
            <div class="input-group">
    ...
    <script>
        ...
        setTimeout(() => {
            window.location.href = "./borrow-page.php"; // Change this to your desired redirect page
