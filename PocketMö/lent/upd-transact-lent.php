...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate recipient name and check for non-zero balance
    $stmt = $conn->prepare("SELECT * FROM lent WHERE Name = ? AND User_Id = ? AND Balance > 0");
    $stmt->bind_param("si", $recipient_name, $userId);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        // Recipient exists with a non-zero balance, proceed with the update
        ...
        if ($new_balance < 0) {
            ...
            $update_stmt = $conn->prepare("UPDATE lent SET Due_Date = ?, Payment_Amount = ?, Balance = ? WHERE ID = ?");
    ...
    // Close the statement
    $stmt->close();
    header("Location: ./lent-page.php"); // Redirect to the lent page
    exit();
}
?>
...
<body>
        ...
        <div class="modal">
            <form id="transaction-form" action="" method="POST">
                ...
                <div class="input-group">
                    ...
                    <input type="text" id="recipient-name" name="recipient_name" placeholder="Enter Recipient's Name" autocomplete="off" required>
                    <div id="suggestions"></div> <!-- Suggestions for recipient names -->
                </div>
    ...
    recipientInput.addEventListener('input', function() {
        ...
        if (query.length > 0) {
            fetch(`./get-recipients.php?query=${encodeURIComponent(query)}`)
    ...
    function handleSuggestionClick(suggestion) {
        recipientInput.value = suggestion.name; // Set the input value to the selected name
        remainingBalanceInput.value = formatBalance(suggestion.balance); // Update the balance input

        clearSuggestions(); // Clear suggestions
    }

    // Add keydown event listener to the recipient input
    ...
    document.addEventListener('click', function(event) {
        if (!suggestionsContainer.contains(event.target) && event.target !== recipientInput) {
